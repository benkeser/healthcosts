...
In this demonstration, we illustrate how algorithms from the `healthcost` repository can be integrated with the existing `SuperLearner` R package. Using simulated data, we illustrate how to implement the algorithms designed specifically for estimating health care costs. We show how the performance of the Super Learner can be evaluated using cross-validation. Finally, we illustrate a function to estimate treatment effects using TMLE. 
